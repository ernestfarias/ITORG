...
RS.Open "SELECT COUNT(*) FROM ittodo", Conn
totaltodo = RS(0)
RS.Close

RS.Open "SELECT COUNT(*) FROM itdiarias", Conn
totaldiarias = RS(0)
RS.Close

MsgBox "Desarrollado por Ernesto A Farias , int. 5065" & vbCrLf & "Ver. :" & App.Major & "." & App.Minor & "." & App.Revision & vbCrLf & vbCrLf & "Estadisticas (" & Now & ")" & vbCrLf & "Total Diarias: " & totaldiarias & vbCrLf & "Total Programadas: " & totaltodo, vbOKOnly
Info
End Sub
...
'    .Fields("propietario").Value = Text2.Text
    ...
    End With
CantAntToDo = CantAntToDo + 1 'agrego 1 al cant para que no le diga al usuario si lo agrego el mismo que tiene tarea nueva
Guardar = False
End If
...
Dim RS As ADODB.Recordset
Set RS = New ADODB.Recordset
RS.Open "SELECT COUNT(*) FROM ittodo where propietario = '" & User & "'", Conn
CantAntToDo = RS(0)
RS.Close
End Sub

Private Sub Timer1_Timer()
'con este timer mira si tengo tareas nuevas y las muestro con respecto hoy usando fcreado
HagoConexion
Dim str As String
Dim RS As ADODB.Recordset
Set RS = New ADODB.Recordset
...
RS.Open "SELECT COUNT(*) FROM ittodo where propietario = '" & User & "'", Conn
CantActual = RS(0) ' cuento registros en el rs
RS.Close

If CantAntToDo < CantActual Then
RS.Open "SELECT * FROM ittodo where propietario = '" & User & "' and fcreado BETWEEN CONVERT(CHAR(10),getdate(),110) AND CONVERT(CHAR(10),DATEADD(dd, 1, getdate()),110)", Conn 'consulto por los agregados el dia de hoy
While Not RS.EOF
str = str & "- " & RS(6) & "  Prioridad:" & RS(2) & vbCrLf
RS.MoveNext
Wend
Form1.Show
MsgBox "Tiene una nueva tarea programada!" & vbCrLf & "" & vbCrLf & str, vbInformation
CantAntToDo = CantActual
Command12_Click
End If
